        ...
        <div className="space-y-2">
          {photos.map((url, idx) => (
            <div
              ...
              className="flex items-center gap-2 p-2 rounded-lg bg-parchemin/5 border border-parchemin/10 cursor-grab active:cursor-grabbing"
            >
...
interface EditGisementsFormProps {
  post: EditableBlogPost;
  lang?: "en" | "fr";
}

function parseGisementsData(raw: unknown): GisementsData {
  const d = raw as Partial<GisementsData>;
  return {
    localityName: d?.localityName ?? "",
    localityLocation: d?.localityLocation ?? "",
    geologicalPeriod: d?.geologicalPeriod ?? "",
    geologicalStages: d?.geologicalStages ?? [],
    introduction: d?.introduction ?? "",
    fauna: d?.fauna ?? [],
  };
}

export default function EditGisementsForm({
  post,
  lang = "fr",
}: EditGisementsFormProps) {
  ...
  const initial = parseGisementsData(post.structuredData);
  ...
  const handleSubmit = async (submitStatus: BlogStatus) => {
    ...
    try {
      ...
      await updateBlogArticle(post.id, {
        title: title.trim(),
        slug: slug.trim(),
        ...
        status: submitStatus,
        publishedAt:
          submitStatus === BlogStatus.PUBLISHED && !post.publishedAt
            ? new Date()
            : undefined,
        tagIds: selectedTags,
      ...
      toast.success(
        submitStatus === BlogStatus.PUBLISHED
          ? "Gisement publié !"
          : submitStatus === BlogStatus.ARCHIVED
            ? "Article archivé"
            : "Brouillon sauvegardé",
      );
      router.push(`/${lang}/blog/${slug.trim()}`);
      router.refresh();
    } catch (err) {
      toast.error(
    ...
    <div className="container mx-auto px-4 py-8 max-w-7xl">
      <div className="grid grid-cols-1 lg:grid-cols-3 gap-6">
            ...
            <CardContent className="pt-6 space-y-4">
              ...
              <div>
                <div className="flex items-center justify-between mb-1">
                  ...
                  >
                    {slugManual
                      ? "Verrouillé — cliquer pour libérer"
                      : "Modifier"}
                  </button>
                </div>
                <Input
          ...
          <Card className="bg-silex border-parchemin/10">
            ...
            <CardContent>
              <Textarea
                ...
                rows={6}
                placeholder="Présentez le site paléontologique..."
                className="bg-silex border-parchemin/20 text-parchemin resize-none"
              />
            </CardContent>
          </Card>
        ...
        <div className="space-y-4">
          ...
          <Card className="bg-silex border-parchemin/10">
            <CardContent className="pt-6 space-y-3">
              ...
              <Button
                type="button"
                variant="outline"
                className="w-full border-parchemin/20 text-parchemin/60"
          ...
          {/* Status badge */}
          <div className="flex items-center gap-2 px-4 py-2 rounded-xl border border-parchemin/10 bg-silex text-sm text-parchemin/60">
            Statut actuel :
            <Badge
              variant="outline"
              className="border-parchemin/20 text-parchemin ml-1"
            >
